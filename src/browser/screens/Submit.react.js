...
class Submit extends Component {
  ...
  submit() {
    ...
    const skills = Object.values(rawCandidate.skills.toObject())
      .filter(item => item)
      .map(item => item.toObject());
    const features = Object.values(this.props.candidate.features.toObject())
      .filter(item => item)
      .map(item => item && item.toObject().desc);
